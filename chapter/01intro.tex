...
In the following example depicted in Figure~\ref{fig:gpt-route-response}, we prompted GPT-4o with a simple navigation task to observe whether the model could successfully provide a useful answer to a simple path finding problem.
The prompt used to generate the response shown in Figure~\ref{fig:gpt-route-response} was as follows:
...
\begin{figure}[h!]
    ...
    \caption{One possible correct route (green) for the example navigation task compared to various segments found in the GPT-4o response (red). Map data \copyright{} 2025 Google.}
    \label{fig:gpt-route-comparison}
\end{figure}

% What is wrong with the LLM response?

Even though the model response shown in Figure~\ref{fig:gpt-route-response} appears structurally sound and plausible at first glance, upon closer inspection there are several mistakes that would likely prevent the user from following the proposed route.
To understand the mistakes, we analyze the response step by step, starting with the first instruction.
To ensure followability, we provide Figure~\ref{fig:gpt-route-comparison} showing one possible route derived using the web-based mapping tool Google Maps as comparison.
...
Also, the response introduces a street called ``Willy-Brandt-Allee'' which does not align with the map context shown in Figure~\ref{fig:gpt-route-comparison}.
...
However, the map shown in Figure~\ref{fig:gpt-route-comparison} suggests no visible connection between ``Willy-Brandt-Allee'' and ``Albersloher Weg''.
...
Further, the response introduces the ``Aasee park area'', which is inconsistent with the route context shown in Figure~\ref{fig:gpt-route-comparison}.
Additionally, the map shown in Figure~\ref{fig:gpt-route-comparison} shows no visible connection between the two segments.
...
Again, this instruction does not appear viable in the example situation, as Figure~\ref{fig:gpt-route-comparison} does not show a visible connection between the two required streets. 
...
Again, these instructions do not appear viable, as the map context shown in Figure~\ref{fig:gpt-route-comparison} does not show a visible connection between the two streets.
...
In total, GPT-4o suggested several steps that may be difficult to follow in practice when compared against the map context shown in Figure~\ref{fig:gpt-route-comparison}.
...
Many potential uses for LLMs in GIScience applications have been theorized, among those the possibility to act as natural language interfaces for spatial queries such as navigation \citep{van_de_weghe_opportunities_2025}.
Several benchmarks indicate that LLMs struggle with planning- and navigation-related tasks \citep{valmeekam_large_2022, ding_mango_2024}.
...
Context enrichment is widely used to provide task-relevant external information to LLMs \citep{lewis_retrieval-augmented_2021, gao_retrieval-augmented_2024}.
...
Since LLMs are primarily trained on text corpora \citep{minaee_large_2025}, we hypothesize that the geographic context should be presented to LLMs in textual form as well.
...
A solution to address this related issue could be to utilize a framework for qualitatively describing topological data such as the dipole calculus \citep{moratz_condensed_2011}.
The dipole calculus is a framework that has been discussed in applications such as robotics \citep{moratz_condensed_2011}.
...
Context enrichment has been shown to make LLM responses less prone to hallucinations in other domains \citep{gao_retrieval-augmented_2024}, while dipole relations provide a proven framework for qualitatively describing topological data \citep{moratz_condensed_2011}.
