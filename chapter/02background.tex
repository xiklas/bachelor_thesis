...
For example, given the input tokens (which can be thought of as words or parts of words) "Roses are", a large language model would attempt to predict the next token, which in this case could be "red".
...
To continue with the previous example, the next tokens after "Roses are red" could be "violet", with the prediction process eventually resulting in the sentence "Roses are red, violets are blue" (for example).



GPT-3 developed by OpenAI in 2020 is considered by many to be the first example of a large language model \citep{minaee_large_2025}.
