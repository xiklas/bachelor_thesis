...
Accordingly, this section provides a high-level overview on LLMs and summarizes their relevant characteristics to this study.
...
For example, given the input tokens (tokens are typically words or subword units) "Roses are", a large language model would generate a plausbile next token such as "red".
...
In the pre-training process, the model parameters are optimized to minimuze a training objective.
This means that the model, often consisting of billions of parameters is adjusted iteratively.
In each iteration, the objective is evaluated and the parameters are updated accordingly.
Using this error, the parameters of the model are adjusted and the process is repeated until predefined stopping criteria are met.
In addition to pre-training, the model can also be fine-tuned to match a specific task or setting \citep{minaee_large_2025}.

Many additionl details regarding LLM architectures and training process are out of the scope of this thesis.
The preceeding overview provides the necessary concepts for the remainder of this thesis.
...
Examples include in-context learning and multi-step problem solving (often referred to as ``reasoning'').
...
``Reasoning'' is commonly used to describe the ability of problem solving requiring multiple intermediate steps, for example math problems \citep{wei_emergent_2022}.
...
To the best of our knowledge, we did not find any publicly documented claims on LLM navigation abilities.
...
This plausibility can distinguish hallucinated outputs from obviously degenerate output.
To reduce hallucinations, techniques have been developed to provide LLMs with additional information after the training process \citep{minaee_large_2025}.
...
Given a user prompts, the system retrieves relevant information from this knowledge base using a retrieval query.
...
The LLM can then generate an output using the previously retrieved knowledge as an additional source \citep{lewis_retrieval-augmented_2021}.
...
Recent work has applied graph-based RAG to LLM-based navigation \citep{moratz_bilateral_2025}.
...
In the example given in Table 2.1, we illustrate the difference between qualitative and quantitative geographic data in the context of our research topic.
...
This yields in a framework allowing for qualitative reasoning on line-based geographic structures such as road networks \citep{moratz_condensed_2011}.
Dipole relations have recently been used as context for LLM-based navigation \citep{moratz_bilateral_2025}.

Although extended versions of the dipole calculus exist, Figure~2.1 shows the set of dipole relations used in this thesis.
In Figure~2.1, each dipole is represented by a unidirectional arrow.
...
A single street may correspond to multpile dipoles depending on the segmentation of its geometry.
...
Within the scope of our literature review, we were not able to identify any studies investigating any dipole-relation based solutions for LLM navigation tasks.
...
Having this background knowledge firmly in place, we continue now by present our experimental methods in the following chapter.