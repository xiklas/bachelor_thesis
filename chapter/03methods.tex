...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{chapter/graphics/experimental_design_diagram.png} % Reduced to 60% of text width
    \caption{Overview of the experimental design used in this study.}
...
While the Test Group performs the exact same tasks as the Control Group, it was ensured that no memories of previous trials were available to the models in the Test Group.
This was achieved by starting each individual trial in a new session, disconnected from any previously conducted trials.
An overview of the experimental design is given in Figure~\ref{fig:experimental-design}.
...
In the following, we refer to a task as a single start-to-end navigation problem, while a trial refers to a single model response to a specific task.
