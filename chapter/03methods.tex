...
\begin{figure}[h!]
    ...
    \caption{Image credit: James Odienki.}
    \label{fig:graphic6}
\end{figure}
...
% How the Chi Squared test was used for significance testing.

Finally, to determine whether the differences in task success rates between the control and test groups were statistically significant, we employed the Chi-Squared ($\chi^2$)test for independence.
This test is commonly used to compare success rates between two independent groups, making it well-suited for our experiments.
The alternative hypothesis in our case states that the inclusion of qualitative geographic context has no significant effect on the navigation performance of LLMs.
We set the significance level to ($\alpha$) = 0.05.
This means that if the resulting p-value from our Chi-Squared test is less than 0.05, we can reject the alternative hypothesis.
Consequently, we can then accept the research hypothesis that the inclusion of qualitative geographic context significantly improves the navigation capabilities of LLMs.
