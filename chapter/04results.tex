...
In Table 4.1, we provide an overview of the two datasets used in our experiments.
We were able to successfully extract symbolic dipole relations for both tested cities.
For the Hamburg dataset, this resulted in a total of 38 individual streets and an average task distance between start and end point of 899 meters as measured via Google Maps.
On the other hand, the Münster dataset contained a total of 128 individual streets, and also had a higher average task distance of 1272 meters as measured via Google Maps.
...
\hline
\end{tabular}
\caption{Overview of experiment results in control and test groups with success and failure counts and corresponding success rates.}
\end{table}
...
Table 4.2 summarizes the overall results of our experiments.
...
The next best performing model was Claude Sonnet 4.5, which achieved a slightly lower task success rate of 65\%.
...
All models using the Hamburg dataset scored at least 75\% task success rate, with Gemini 2.5 Pro ranking the highest at 95\%.
Claude Sonnet 4.5 followed with a task success rate of 90\%, a decrease of 5\%, and GPT-4o achieved a task success rate of 75\%, which is 20\% lower than Gemini 2.5 Pro.
...
Gemini 2.5 Pro again ranked highest among the three with a task success rate of 45\%, followed by Claude Sonnet 4.5 at 40\% and GPT-4o at 30\%.
The lowest overall task success rate in the test group was thus observed with GPT-4o using the Münster dataset at 30\%, while the highest overall task success rate was achieved by Gemini 2.5 Pro using the Hamburg dataset at 95\%.
...
The above Table 4.3 summarizes how many navigation tasks were answered correctly by how many models in this group.
Only six navigation tasks were answered incorrectly by all three models.
Counting the occurernces of navigation tasks where just one model gave a correct answer, we find that there are nine such tasks.
...
Unsurprisingly, the percentage of navigation tasks where at least zero models answered correctly is 100\%.
...
Although there are just nine such cases overall and the results deviate by just two between the best and worst performing model, the winner in this category is also Gemini 2.5 Pro.
It answered four navigation tasks correctly where both other models failed.
GPT-4o then followed with three such cases.
Claude Sonnet 4.5 performed worst in this category, answering just two navigation tasks correctly when no other model could. 
...
out of the nine occurrences where just one model failed, GPT-4o was responsible for seven of these failures.
Both Gemini 2.5 Pro and Claude Sonnet 4.5 were only responsible for one failure of this kind each.
...
Next, we may analyze the failures in the test group by the dataset they occurred on.
While failures where just one model failed to answer a navigation task occurred almost equally in both cities, the results look different for failures by two or even three models.
In Hamburg, there were 4 occurrences where one model failed to answer a navigation task.
In Münster, this number was just slightly higher at 5.
In the next category however, where two models failed to answer a navigation task, there were two such occurrences in Hamburg and seven in Münster.
...
In Münster however, there were six such occurrences.
Counting together the occurrences of failures, we find that six out of a total 24 failure cases occurred in Hamburg, while 18 occurred in Münster.
As a percentage, this means that 25\% and 75\% of failures occurred in Hamburg and Münster respectively.
Shifting our view at failure cases where at least two models failed, just two out of a total of 15, or 13.33\% of such cases occurred in Hamburg, while 13 out of 15, or 86.67\% occurred in Münster.
...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Model Pair} & \textbf{Disagreement Frequency (Out of 18 Trials)} & \textbf{Agreement Frequency} \\
\hline
...
\hline
\end{tabular}
\caption{\textbf{Pairwise Model Disagreement and Agreement in Non-Unanimous Trials}}
\label{tab:pairwise_disagreement}
\end{table}

Finally, we may analyze the pairwise disagreement performance of the three studied models.
Setting aside the 22 cases where all models either answered correctly or incorrectly, we are left with 18 trials where the results were non-unanimous.
In these trials, we can analyze how often two models disagreed with each other, meaning that one model answered correctly while the other answered incorrectly or the other way around.
The results of this analysis are summarized in Table 4.8.
Out of the 18 non-unanimous trials, GPT-4o and Gemini 2.5 Pro disagreed in 15 cases, while agreeing on 3 cases.
...
This means that Gemini 2.5 Pro and Claude Sonnet 4.5 agreed more often than they disagreed.
On the other hand, any combination involving GPT-4o resulted in more disagreements than agreements.

\section{Summary of Findings}

In summary, our experiments revealed that LLM navigation performance with additional symbolic geographic context was higher than without it.
