...
The overall success rate increased from 0/120 (0\%) in the control group to 75/120 (62.5\%) in the test group (see Table~4.1).
...
Although certainly encouraging, a success rate of 62.5\% is therefore not sufficient for many use cases, when tradiitional navigation solutions operate at near perfection.
...
Areas of poor performance today may be addressed by a new model tommorow.
...
Additionaly, the climate impact of LLM usage has been an ongoing topic of disussion.
Knowing more about LLM capabilities and limitations could help users make concsious decision about when to employ an LLM and when to rely on other tools.
...
Some recent products tried to make LLM-based AI a key selling point, only to underdeliver and be met with harsh user criticism.
...
It doesn't intent to show that LLMs are ready to be a viable alternative to traditional navigation solutions yet, but that doesn't mean that the idea can be fully dismissed either.
